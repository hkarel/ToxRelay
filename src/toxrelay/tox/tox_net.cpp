...
//    QT_TRANSLATE_NOOP("ToxNet", "The ToxPhone client not support a chat function");

static const char* tox_transfer_data_message =
    QT_TRANSLATE_NOOP("ToxNet", "The ToxRelay client not support a transfer of data");
...
ToxNet::ToxNet() : QThreadEx()
{
    ...
    _configFile = _configPath + "toxrelay.tox";
...
//    QString avatarFile = "://toxrelay.png";
...
//    }
    ...
    if (file.open(QIODevice::ReadOnly))
    {
        ...
        log_error_m << "Failed open a tox avatar file ://toxrelay.png";
    ...
    if (uint32_t friendCount = tox_self_get_friend_list_size(_tox))
    {
        ...
        tox_self_get_friend_list(_tox, fnumbers.data());

        log_info_m << "Friends list:";
        for (uint32_t i = 0; i < friendCount; ++i)
        {
            uint32_t friendNumber = fnumbers[i];
            QString friendName = getToxFriendName(_tox, friendNumber);
            QByteArray friendPk = getToxFriendKey(_tox, friendNumber);
            TOX_CONNECTION connectStatus = getFriendConnectStatus(_tox, friendNumber);
            log_info_m << "  Name: "       << friendName
                       << ". PubKey: "     << friendPk.toHex().toUpper()
                       << ". Connection: " << toString(connectStatus);
        }
    }
                                ...
                                void* user_data)
{
    ...
    friendRequest.publicKey = QByteArray((char*)pub_key, TOX_PUBLIC_KEY_SIZE);
                                ...
                                const uint8_t* message, size_t length, void* user_data)
{
    if (type == TOX_MESSAGE_TYPE_NORMAL)
    {
        ...
        toxMessage.text = QString::fromUtf8(QByteArray((char*)message, length));
...
//        }
        ...
        // Отменяем получение аватара от друга, аватар не нужен консольной утилите
    ...
    else
    {
        data::ToxMessage toxMessage;
        toxMessage.friendNumber = friend_number;
        toxMessage.text = QObject::tr(tox_transfer_data_message);
